...
---
# 동물원 관람 지원(Zoo Visit Guide) AI 에이전트 개발 계획서

본 문서는 샘플 설계서의 구조를 동물원 관람 지원 업무를 위한 AI Agent 개발 계획서다.

외부 계약이 필요한 기능은 결정적인 Mock 데이터로 구현하고, Local PC에서 개발·테스트·시연까지 완료하는 범위로 제한한
...
### 4.1 AI Agent 전체 작동 원리 (RAG + 조회 Tool, P0)
...
> ⚠️ 위 다이어그램은 전체 UX를 한눈에 보기 위한 상위 흐름이라, 예약(P1) 부분은 `H → J → K/L/M/L2` 4~5개 노드로 압축되어 있습니다. 실제 화면 전환·버튼·타이머까지 구현하려면 아래 4.4절의 상세 흐름을 따로 참조해야 합니다.

### 4.4 예약 승인 사용자 경험 흐름 (상세)

> 4.3절의 "예약(P1)" 분기 하나를 화면 상태·버튼·타이머 단위로 펼친 상세 흐름입니다. 4.2절(Backend 시퀀스)과 반드시 1:1로 대응해야 하며, Frontend 구현 시 이 다이어그램을 화면 상태 목록으로 그대로 사용합니다.본 문서는 샘플 설계서의 구조를 동물원 관람 지원 업무를 위한 AI Agent 개발 계획서다.
>
> 외부 계약이 필요한 기능은 결정적인 Mock 데이터로 구현하고, Local PC에서 개발·테스트·시연까지 완료하는 범위로 제한한다.

```mermaid
flowchart TD
    subgraph UX["Streamlit 화면 상태"]
        R1[예약 요청 메시지 입력]
        R2{프로그램·시간·인원이<br/>모두 확인됐는가?}
        R3[부족한 항목만 되묻는 채팅 말풍선]
        R4["예약 확인 카드 표시<br/>(프로그램/시간/인원/유의사항)"]
        R5["카드에 남은 시간 카운트다운<br/>(TTL 120초)"]
        R6{{"사용자 버튼 선택"}}
        R7[처리 중 스피너 표시]
        R8[예약 완료 카드<br/>예약번호 표시]
        R9[소유권 오류 안내<br/>사유는 노출하지 않음]
        R10[취소 완료 메시지]
        R11[만료 안내 · 다시 확인 유도]
        R12[일시적 오류 안내 · 재시도 유도]
    end

    subgraph BE["Backend 판정 (4.2절과 대응)"]
        B1["change Tool 감지<br/>Pending Action 저장"]
        B2{"confirm 요청의<br/>session_id 일치?"}
        B3{"approval_status == pending<br/>이고 TTL 이내인가?"}
        B4["approval_status → processing<br/>reserve_experience_program 실행"]
        B5["approval_status → completed"]
        B6["approval_status → rejected"]
    end
    ...
    R6 -->|"확인" 클릭, TTL 이내| R7
    R6 -->|"취소" 클릭| R10
    R5 -.타이머 만료 시 자동 전환.-> R11

    R7 --> B2
    B2 -->|불일치| B6 --> R9
    B2 -->|일치| B3
    B3 -->|"pending 아님(중복 확인 등)"| B6 --> R11
    B3 -->|"만료됨"| B6 --> R11
    B3 -->|유효| B4
    B4 -->|Tool 실행 성공| B5 --> R8
    B4 -->|Tool 실행 실패<br/>재고·정원 부족 등| B6 --> R12
...
**이 다이어그램에서 4.3절 상위 흐름과 달리 새로 드러나는 지점:**

- **R5(카운트다운) → R6 사이의 점선 경로**: 사용자가 아무 버튼도 누르지 않은 채 TTL(120초)이 지나면, 백엔드에 재요청을 보내지 않고도 **화면이 스스로 "만료" 상태(R11)로 전환**되어야 합니다. 그렇지 않으면 사용자가 이미 만료된 카드에 대고 "확인"을 눌러 불필요한 실패 요청이 발생합니다.
- **R6 → R7(처리 중 스피너)**: v0.2/4.3절 어디에도 "확인 버튼을 누른 직후"의 화면 상태가 없었습니다. `confirm` API 응답이 오기 전까지 버튼을 다시 누르지 못하도록 스피너로 잠가야, A-08(같은 action_id 동시 확인)의 프론트엔드 쪽 원인을 사전에 줄일 수 있습니다.
- **B2(세션 불일치) → R9**: 4.3절 상위 흐름에는 "소유권 오류"가 예약 분기 안에 `L2` 하나로만 있었는데, 이번 상세 흐름에서는 이것이 **일반 실패(R11 만료·R12 오류)와 분리된, 사용자에게 사유를 노출하지 않는 별도 상태**라는 점을 명확히 했습니다(9.2절·12장의 소유권 검증 원칙과 연결).
- **B4 실행 실패(R12) 분기**: v0.2/4.3절에는 "승인까지는 통과했지만 Tool 실행 자체가 실패하는 경우"(예: 확인하는 그 짧은 순간 정원이 소진됨)가 빠져 있었습니다. 이 경우는 소유권·TTL 문제가 아니므로 "다시 확인해 주세요"가 아니라 **"일시적 오류이니 재시도"** 안내로 구분했습니다.
...
| P0은 In-Memory 저장소                  | Redis/PostgreSQL 연동은 P1, Repository 인터페이스만 미리 분리                      |
...
| 제한된 Multimodal                      | P0 범위 밖, P1에서도 선택 기능으로 취급                                            |
