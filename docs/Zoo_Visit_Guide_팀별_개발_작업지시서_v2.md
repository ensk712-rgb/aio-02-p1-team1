# Zoo Visit Guide — 팀별 개발 작업지시서 v2

작성일: 2026-09-07 · 대상: 손영민 / 최두나 / 이원민 · 목적: 수업 내용을 적용한 로컬 AI Agent 시연과 충돌을 줄이는 분담 개발 · 공통 Python: 3.12.7
...
- 충돌 시 기준: `동물원_관람_지원(Zoo_Visit_Guide)_AI_에이전트_개발_계획서_v3.md`**의 기능·정책을 우선하고, 명시되지 않은 구현 방법·담당 경계는 이 문서로 확정한다.** 기준 문서의 내부 불일치는 13장에 보정 결정을 기록했다.
...
└─ docs/
   ├─ Zoo_Visit_Guide_팀별_개발_작업지시서_v2.md
   ├─ 동물원_관람_지원(Zoo_Visit_Guide)_AI_에이전트_개발_계획서_v0.3.md                                     
   └─ reports/
```
